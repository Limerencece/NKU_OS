...
static inline void page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) //删除虚拟地址 la 的页表映射
{
    if (*ptep & PTE_V)  // 如果该 PTE 有效
    { //(1) check if this page table entry is
        struct Page *page =
            pte2page(*ptep); //(2)  根据 PTE 找到对应的物理页对象
        page_ref_dec(page);  //(3) 减少该物理页的引用次数
        if (page_ref(page) ==
            0)
        ...
        *ptep = 0;                 //(5) clear second page table entry
        tlb_invalidate(pgdir, la); //(6) flush tlb 清除 TLB 中的旧缓存
    }
}
...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) //pgdir是页表基址(satp)，page对应物理页面，la是虚拟地址
{
    ...
    page_ref_inc(page); //增加物理页的引用次数
    if (*ptep & PTE_V) //如果该 PTE 已存在映射
    {
        struct Page *p = pte2page(*ptep);
        if (p == page)
        {
            page_ref_dec(page); // 如果是同一个物理页，减少引用次数
        }
        else
        {
            page_remove_pte(pgdir, la, ptep); // 否则删除旧映射
        }
    }
